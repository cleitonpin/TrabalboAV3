...
def cria():
    ...
    while continua == 1:
        ...
        telefone = 0
        data_nascimento = input('Insira sua data de nascimento [+14] [dd/mm/yyyy]: ')

        if data.valida(data_nascimento):
            print('ERROR: FORMATO DA DATA INVÁLIDO')
        ...
        date2 = datetime.strptime(data_nascimento, '%d/%m/%Y')
        ...
        if row[0] > 0 or row[1] > 0:
            ...
            bd1.cursor.execute("insert into usuarios(telefone,email,usuario,nick,data_nascimento,senha) values(%s,%s,%s,%s,%s,%s)",(telefone,email,usuario,nick,data_nascimento,senha))
